...
//! one, inlined here over any `tokio` stream. It drives every coroutine
//! the same way.
...
/// cases. Graceful stop is by aborting the task, so this never polls the
/// shutdown flag itself.
...
/// refresh timeout), then ends IDLE cleanly. Standalone so the IDLE-only
/// watcher can compose it.
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    loop {
        // NOTE: let a shutdown request wind IDLE down cleanly (send
        // DONE).
        if shutdown.load(Ordering::SeqCst) {
            done.store(true, Ordering::SeqCst);
        }
        match idle.resume(fragmentizer, arg.take()) {
            // NOTE: untagged data; end IDLE, then resync.
            ...
            ImapCoroutineState::Yielded(ImapIdleYield::WantsRead) => {
                match timeout(IDLE_REFRESH, stream.read(&mut buf)).await {
                    ...
                    Ok(Ok(n)) => arg = Some(&buf[..n]),
                    Ok(Err(err)) => return Err(err).context("read failed"),
                    // NOTE: refresh window elapsed; drop and reconnect.
...
/// Drives an IDLE coroutine whose `done` flag is already set to
/// completion (writing DONE, reading the tagged response).
...
/// QRESYNC (e.g. Gmail, Yahoo). Tracks new messages only: it keeps the
/// mailbox's `UIDNEXT`, and on each IDLE wake re-EXAMINEs and `UID
/// FETCH`es any UIDs that appeared since, emitting one `new` event each.
/// Flag changes and deletions are not tracked (that needs the
/// `run_watch` path). Read-only throughout.
///
/// Returns `Ok(())` on a clean wind-down or the periodic refresh, `Err`
...
where
    S: AsyncRead + AsyncWrite + Unpin,
{
    // NOTE: baseline is the UID the next new message will get; anything
    // >= this that later appears is new mail.
    ...
    loop {
        ...
        if new_next <= next_uid {
            // NOTE: woke for something other than new mail (a flag change
            // / expunge we don't track without QRESYNC).
            continue;
        }

        // NOTE: fetch just the new UIDs; a bounded range (not `:*`)
        // avoids the "UIDNEXT:* returns the last message" gotcha.
